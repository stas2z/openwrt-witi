...
ifeq ($(strip $(CONFIG_EXTERNAL_KERNEL_TREE)),"")
  ifeq ($(strip $(CONFIG_KERNEL_GIT_CLONE_URI)),"")
	...
	rm -rf $(LINUX_DIR); \
	mv -f $(KERNEL_BUILD_DIR)/linux-3.10-witi-$(LINUX_VERSION) $(LINUX_DIR); \
	ln -sf $(LINUX_DIR) $(KERNEL_BUILD_DIR)/linux-kernel; \
	else \
	if [[ "$(LINUX_SOURCE)" == *.tar.bz2 ]];then \
	    bzcat $(DL_DIR)/$(LINUX_SOURCE) | $(TAR) -C $(KERNEL_BUILD_DIR) $(TAR_OPTIONS); \
	    rm -rf $(LINUX_DIR); \
	    mv -f $(KERNEL_BUILD_DIR)/sta-s2z-linux-$(LINUX_VERSION) $(LINUX_DIR); \
	    ln -sf $(LINUX_DIR) $(KERNEL_BUILD_DIR)/linux-kernel; \
	else \
