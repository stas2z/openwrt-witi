...
define KernelPackage/mtk-hwcrypto
  CATEGORY:=MTK Properties
  SUBMENU:=Drivers
  DEPENDS:=+kmod-ipsec4
  TITLE:=Mediatek HW crypto engine
  KCONFIG:=	CONFIG_RALINK_HWCRYPTO=m \
		CONFIG_HW_IPSEC=y

  FILES:=$(LINUX_DIR)/drivers/net/cryptoDriver/crypto_k.ko
  AUTOLOAD:=$(call AutoLoad,90,crypto_k)
endef

define KernelPackage/mtk-hwcrypto/description
 Kernel module for Mediatek HW Crypto Engine (IPSEC).
endef

$(eval $(call KernelPackage,mtk-hwcrypto))
