...
ucidef_set_interface_wan2() {
	local ifname=$1

	uci batch <<EOF
set network.wan2='interface'
set network.wan2.ifname='$ifname'
set network.wan2.proto='dhcp'
set network.wan26='interface'
set network.wan26.ifname='@wan2'
set network.wan26.proto='dhcpv6'
EOF
}
...
ucidef_set_interfaces_lan_wan_wan2() {
	local lan_ifname=$1
	local wan_ifname=$2
	local wan2_ifname=$3
	...
	ucidef_set_interface_lan "$lan_ifname"
	ucidef_set_interface_wan "$wan_ifname"
	ucidef_set_interface_wan2 "$wan2_ifname"
}
