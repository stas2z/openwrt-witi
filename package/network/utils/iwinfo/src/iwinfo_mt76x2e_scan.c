...
int mt76x2e_get_scanlist(const char *ifname, char *buf, int *len)
{
	...
	if( mt76x2e_ioctl(ifname, SIOCGIWRANGE, &wrq) >= 0 )
	{
		...
		/* Initiate Scanning */
		if( mt76x2e_ioctl(ifname, SIOCSIWSCAN, &wrq) >= 0 )
		{
			timeout -= tv.tv_usec;
		...
		realloc:
					...
					if( mt76x2e_ioctl(ifname, SIOCGIWSCAN, &wrq) )
					{
			...
			if( wrq.u.data.length )
			{
				...
				do
				{
					...
					ret = mt76x2e_extract_event(&stream, &iwe, range.we_version_compiled);
					...
					if(ret >= 0)
					{
						...
						mt76x2e_fill_entry(&stream, &iwe, &range, has_range, &e);
					}
